...
export class EmplistComponent implements OnInit {
  ...
  get Supplier(): any { return this.EmployeelistForm.get('Supplier'); }

  get SupplierList(): any {
    return this.EmployeelistForm.get('SupplierList') as FormArray;
  }


  ngOnInit(): void {
    this.Suppliers = this.employeeService.GetSupplierAutoCompleteData();
    this.GetVendorList();

    this.EmployeelistForm.valueChanges.subscribe(
      () => {
        this.GetVendorList();
      });
  }
  ...
  public GetVendorList(): void {
    this.employeeService.GetVendorList().subscribe(
      data => {
  ...
  selectEvent(item: any) {
    ...
    this.employeeService.GetVendor(item.VID).subscribe(
      data => {
        console.log();
        if (data.length > 0) {
          const controls = <FormArray>this.EmployeelistForm.controls['SupplierList'];
  ...
  public Delete(index: any, VID: any): void {
    if (VID) {
      if (confirm('Are you sure to delete this record')) {
        this.employeeService.Delete(VID).subscribe(res => {
          if (res == null) {
  ...
  openModal(index: any, VID: any) {
    this.employeeService.GetVendor(VID).subscribe((data: any[]) => {
      const initialState = {
      ...
      this.bsModalRef.content.action.subscribe((data: any) => {
        this.suppplier = data;
        this.EmployeelistForm.get('VID').setValue(this.suppplier.VID);
      });
    });
  }
